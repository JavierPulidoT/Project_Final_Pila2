...
import ProjectModel from '../models/ProjectsModels';
...
const index = async (req, res) => {
  ...
  // db.projects.find()
  try {
    log.info('Listando Registros Cryptos...⌛');
    const projectsDocs = await ProjectModel.find();
    log.info('Cryptos listados con exito... 🎉');
    res.json(projectsDocs);
  } catch (error) {
    log.error(`💥 Error al Guardar Registros: ${error.message}`);
...
const addPost = async (req, res) => {
  ...
  try {
    log.info('Salvando el Registro...⌛');
    // Se salva el documento projecto
    project = await projectModel.save();
    log.info('🎉 Registro Crypto salvado con exito 🎉');
    // Redireccionando al recurso que lista los proyectos
    // GET /projects
    return res.redirect('/projects');
  } catch (error) {
    log.error(`Ha fallado el intento de salvar un Registro:${error.message}`);
